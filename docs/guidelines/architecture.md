...
| `StatCard` / `CompactStatCard` | Ver `StatCard.types.ts`                             | Tarjeta de KPI: ícono + chip de tendencia o etiqueta, valor destacado y footer comparativo. `tone: 'error'` suma borde y halo de acento. `loading` reemplaza el valor por un skeleton de la misma altura. La variante condensada es una sola fila. Presentacionales: el valor llega ya formateado |
