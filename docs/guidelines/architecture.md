...
| `StatCard` / `CompactStatCard` | Ver `StatCard.types.ts`                             | Tarjeta de KPI: ícono + chip de tendencia o etiqueta, valor destacado, aclaración opcional (`note`) y footer comparativo. `tone: 'error'` suma borde y halo de acento. La variante condensada es una sola fila. Presentacionales: el valor llega ya formateado      |
...
| `orders`        | Listado global de órdenes (`/orders`) y el detalle de una orden con el ciclo de vida de su envío (`/orders/:orderId`)                                                                                                                                      |
