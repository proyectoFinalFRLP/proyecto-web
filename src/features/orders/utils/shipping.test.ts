...
import { toCreateOrderPayload, toQuotePayload, warehouseCoverage } from './shipping'
...
describe('toQuotePayload', () => {
  it('only tells the quote where the package leaves from', () => {
    expect(toQuotePayload({ warehouseId: 3, name: 'CD Ezeiza' })).toEqual({
      quote: { origin_warehouse_id: 3 },
    })
  })
})
